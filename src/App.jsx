...
const TR = {
  ptBR: {
    ...
    enhancerDesc:
      "Enhancers concedem +100% de Influência Mercante ao entregar um Primed Tradepack. São obtidos por SORTE ao gastar QUEST no jogo — não é uma compra garantida, é uma chance que consome QUEST a cada tentativa. Armazenados como cargas, capacidade máxima de 100. Cada pack Enhanced consome 1 carga.",
    enhancerGanhosLabel: "Cargas obtidas essa semana",
    enhancerGanhosHint: "Sorte ao gastar QUEST no jogo",
  ...
  en: {
    ...
    enhancerTitle: "Primed Tradepack Enhancers",
    enhancerDesc:
      "Enhancers grant +100% Merchant Influence when delivering a Primed Tradepack. They're obtained by LUCK when spending QUEST in-game — not a guaranteed purchase, it's a chance that consumes QUEST per attempt. Stored as charges, capacity max 100. Each Enhanced pack consumes 1 charge.",
    enhancerGanhosLabel: "Charges gained this week",
    enhancerGanhosHint: "Luck roll when spending QUEST",
...
export default function App() {
  ...
  // De qtdPacks/semana, quantos são Tradepack PRIME (usam certificado; só
  // packs Prime podem receber Enhanced). O restante (qtdPacks - qtdPacksPrime)
  // são packs Normais: sem custo de certificado e sem opção de Enhanced.
  const [qtdPacksPrime, setQtdPacksPrime] = useState(10);
  ...
  // Primed Tradepack Enhancers — concedem +100% de Influência Mercante ao
  // entregar um Primed Tradepack (confirmado pelo tooltip oficial do jogo).
  // Obtidos por SORTE ao gastar QUEST no jogo — não é uma compra garantida,
  // é uma chance que consome QUEST a cada tentativa. Armazenados como cargas
  // com capacidade máxima de 100. enhancerStock é o estoque persistido;
  // enhancerGanhos são as cargas obtidas nesta semana, que se fundem
  // automaticamente ao estoque (ver useEffect de consumo abaixo).
  ...
  const applySettings = (s) => {
    ...
    if (s.poolRate !== undefined) setPoolRate(s.poolRate);
        ...
        // Migração: usuários antigos não têm qtdPacksPrime salvo — como o app
        // sempre tratou 100% dos packs como Prime até agora, o padrão pra quem
        // já tinha dados salvos é assumir todos os packs como Prime (mesmo
        // comportamento de antes), em vez de zerar e quebrar o cálculo.
        if (s.qtdPacksPrime !== undefined) setQtdPacksPrime(s.qtdPacksPrime);
        else if (s.qtdPacks !== undefined) setQtdPacksPrime(s.qtdPacks);
  ...
  const r = useMemo(() => {
    ...
    // Só packs PRIME pagam certificado e podem receber Enhanced. O resto
    // (packsSemanais - packsPrimeSemanais) são packs Normais: sem custo de
    // certificado, sem opção de Enhanced — mesma fórmula de Silver/IM base
    // (a fórmula da Tavernlight é por distância, não por tipo de pack).
    const packsPrimeSemanais = Math.min(qtdPacksPrime, packsSemanais);
    const packsNaoPrime = packsSemanais - packsPrimeSemanais;
    ...
    // passar do que há disponível — e só pode vir de packs Prime.
    const enhancerDisponivel = Math.min(100, enhancerStock + enhancerGanhos);
    const enhancedVal = Math.min(qtdEnhanced, packsPrimeSemanais, enhancerDisponivel);
    const enhancerRestante = Math.max(0, enhancerDisponivel - enhancedVal);
    const packsPrimeSemEnhance = packsPrimeSemanais - enhancedVal;
    const packsNormais = packsNaoPrime; // mantido p/ compatibilidade — agora = packs SEM Prime

    const imBase = imPorPack;
    // Packs Normais NÃO geram IM — só geram Silver. A Influência Mercante só
    // vem dos certificados Prime (packs Prime, com ou sem Enhanced).
    const imNaoPrimeTotal = 0;
    const imPrimeTotal = packsPrimeSemEnhance * imBase;
    // Enhanced: confirmado pelo tooltip oficial do jogo — concede bônus de
    // 100% na Influência Mercante obtida ao entregar o Primed Tradepack (×2).
    const ENHANCED_IM_MULT = 2;
    const imEnhancedTotal = enhancedVal * imBase * ENHANCED_IM_MULT;
    const imNormalTotal = imNaoPrimeTotal + imPrimeTotal; // mantido p/ compatibilidade
    const imTotal_semana = imNaoPrimeTotal + imPrimeTotal + imEnhancedTotal;
    ...
    // Só packs Prime pagam certificado — packs Normais não usam certificado.
    const questCerts_sem = packsPrimeSemanais * certCusto_Q;
    ...
    // Enhanced dobra o IM (bônus oficial de +100%) — um pack Enhanced precisa
    // de metade do IM base pra atingir o mesmo break-even.
    const imBreaakevenEnhanced = poolRate > 0 ? questNecessarioPorPack / (poolRate * ENHANCED_IM_MULT) : 0;
  ...
  // Auto-consumo de cargas de Enhancer — funde os ganhos da semana (cargas
  // compradas com QUEST no jogo) no estoque persistido e desconta 1 carga por
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      setEnhancerStock((prevStock) => {
        ...
        // Enhanced só pode vir de packs Prime — nunca mais do que min(qtdPacks, qtdPacksPrime).
        const consumido = Math.min(qtdEnhanced, Math.min(qtdPacks, qtdPacksPrime), disponivel);
        return Math.max(0, disponivel - consumido);
      });
      if (enhancerGanhos !== 0) setEnhancerGanhos(0);
    }, 2000);
    return () => clearTimeout(t);
  }, [qtdEnhanced, enhancerGanhos, qtdPacks, qtdPacksPrime, settingsLoaded]);
      ...
      {tab === "tradepack" && (
        <div>
          <FormulaPanel
            ...
            descricao={
              lang === "en"
                ? "Silver per pack is calculated from the official Tavernlight formula (base + distance × coefficient), adjusted by a patch 1.0.9.5 factor (recalibrated 07/27/2026 from real delivery data, ≈+247% instead of the originally extrapolated +340%), scaled by the destination's Local Demand % (shown at the Tradepost before crafting), then by any active Modifiers (Bartering, Plundering, Stolen). Not every pack is Prime: Normal packs generate Silver only (0 IM) and never pay a certificate or get Enhanced. Merchant Influence (IM) is generated only by Prime certificates, at 10× the pack's Silver value; applying an Enhancer (a luck roll that costs QUEST per attempt, not a guaranteed purchase) grants +100% IM (×2) on that pack's delivery — confirmed by the official in-game tooltip."
                : "O Silver por pack é calculado pela fórmula oficial da Tavernlight (base + distância × coeficiente), ajustada por um fator do patch 1.0.9.5 (recalibrado em 27/07/2026 com dado real de entrega, ≈+247% em vez do +340% extrapolado originalmente), escalada pela Demanda Local % do destino (exibida no Tradepost antes de craftar), e depois pelos Modificadores ativos (Bartering, Plundering, Stolen). Nem todo pack é Prime: packs Normais geram só Silver (0 IM) e nunca pagam certificado nem podem ser Enhanced. A Influência Mercante (IM) só é gerada pelos certificados Prime, em 10× o valor de Silver do pack; aplicar um Enhancer (sorte que consome QUEST a cada tentativa, não é compra garantida) concede +100% de IM (×2) na entrega daquele pack — confirmado pelo tooltip oficial do jogo."
            }
            formulas={[
              {
              ...
              },
              {
                label:
                  lang === "en"
                    ? "Normal vs Prime packs (Normal: Silver only, 0 IM, no certificate, no Enhanced)"
                    : "Packs Normais vs Prime (Normais: só Silver, 0 IM, sem certificado, sem Enhanced)",
                formula:
                  "packsNaoPrime = packsSemana − min(qtdPacksPrime, packsSemana)\nIM(packsNaoPrime) = 0  ·  IM só vem dos certificados Prime\ncustoCertSemana = packsPrime × custoCert (0 para packs Normais)",
              },
              {
                label:
                  lang === "en"
                    ? "Merchant Influence per pack (Prime packs only — Normal packs generate 0 IM)"
                    : "Influência Mercante por pack (só packs Prime — packs Normais geram 0 IM)",
                formula: "IM = Silver × 10  (packs Normais: IM = 0)",
              },
              {
              ...
              },
              {
                ...
                formula:
                  "disponível = min(100, estoque + ganhosSemana)\nenhancedVal = min(qtdEnhanced, packsPrime, disponível)\nrestante = disponível − enhancedVal",
                nota:
                  lang === "en"
                    ? "Enhancers are obtained by LUCK when spending QUEST in-game (official tooltip) — not a guaranteed purchase, it's a chance that consumes QUEST per attempt — and can only be applied to Prime packs. Max capacity is 100 charges; 1 charge is consumed per Enhanced pack. Applying an Enhancer grants +100% IM (×2) on that pack's delivery — confirmed by the official in-game tooltip."
                    : "Enhancers são obtidos por SORTE ao gastar QUEST no jogo (tooltip oficial) — não é uma compra garantida, é uma chance que consome QUEST a cada tentativa — e só podem ser aplicados em packs Prime. Capacidade máxima de 100 cargas; 1 carga é consumida por pack Enhanced. Aplicar um Enhancer concede +100% de IM (×2) na entrega daquele pack — confirmado pelo tooltip oficial do jogo.",
              },
            ]}
          />
          ...
          >
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 8 }}>
              <Field
                ...
                hint={
                  lang === "en"
                    ? "Total packs delivered per week (Normal + Prime)"
                    : "Total de packs entregues por semana (Normais + Prime)"
                }
              />
              <Field
                label={lang === "en" ? "Prime packs / week" : "Packs Prime / semana"}
                value={qtdPacksPrime}
                onChange={(v) => setQtdPacksPrime(Math.min(v, qtdPacks))}
                step={1}
                hint={
                  lang === "en"
                    ? "Of the packs above, how many are Prime (use certificate). The rest are Normal packs — no certificate, no Enhanced."
                    : "Dos packs acima, quantos são Prime (usam certificado). O restante são packs Normais — sem certificado, sem Enhanced."
                }
              />
              <Field
                ...
                hint={
                  lang === "en"
                    ? "10 certificates per pack — only charged for Prime packs"
                    : "10 certificados por pack — cobrado só nos packs Prime"
                }
              />
              <Field
                ...
                hint={
                  lang === "en"
                    ? `${TR[lang].enhancerApplyHint} (Prime packs only)`
                    : `${TR[lang].enhancerApplyHint} (só packs Prime)`
                }
              />
            </div>

            {/* Breakdown Normal vs Prime vs Enhanced */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 8 }}>
              <Stat
                label={lang === "en" ? "Normal packs (no cert.)" : "Packs Normais (s/ cert.)"}
                value={fmtInt(r.packsNaoPrime)}
              />
              <Stat
                label={lang === "en" ? "Prime packs (not Enhanced)" : "Packs Prime (s/ Enhanced)"}
                value={fmtInt(r.packsPrimeSemEnhance)}
                color={gold}
              />
              <Stat
                label={lang === "en" ? "Prime + Enhanced packs" : "Packs Prime + Enhanced"}
                value={fmtInt(r.enhancedVal)}
                color={green}
              />
            </div>

            {/* Primed Tradepack Enhancers — estoque de cargas compradas com QUEST */}
          ...
          <FormulaPanel
            ...
            formulas={[
              ...
              },
              {
                ...
                formula:
                  "imTotal = imExpedição + (packsNormais × imBase) + (packsPrime × imBase) + (packsPrimeEnhanced × imBase × 2)",
              },
              {
